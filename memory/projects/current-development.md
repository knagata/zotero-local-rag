...
- 150行超の関数は22件から17件へ減少。最大は
  `index_from_zotero._index_library`（1,104行）、次が`_extract_pdf_chunks`（679行）。
- 既定suiteは1,530 passed / 4 skipped / 5 deselected。実資料取込baselineは5 passed。
  抽出・取り込み・構造を変更したら
...
1. Mistral deferのartifact payload・manifest保存・既存chunk保持を、extractorと実DBを
   呼ばず統合テストする。経験則・threshold・公開出力は変えない。
2. その後にOCR audit outcome、born-digital/scan-derived/corrupted頁patchの順で、
