...
- [x] **ページ内部IDを章見出しにしない。** 174頁すべてに`00000___<32 hex>`形式の
  ブックマークを持つPDFが、167個の`chapter`として`exact`採用され、156個の偽章要約を
  生成していた。過半数が連番＋hash又はファイル名のアウトラインを、意味的目次として
  一括拒否する。`chapter`だけにあった旧ファイル名検査も共通ゲートへ移し、完全な
...
### 2026-08-15 Herdrによる長時間コマンドの可視化

- [x] **Codexの長時間作業を可視ペーンで実行する。** `CLAUDE.md`に、Herdr管理下での
  全件テスト、coverage、build、server、monitorは現タブの専用ペーンへ分け、フォーカスを
  奪わず、作業ディレクトリを引き継ぎ、ペーンIDと結果を報告する規則を追加した。
- [x] **検証。** 実際に現タブのHerdrペーン`w1:p2`を`--no-focus`で作成し、同一cwdで
  文書参照テスト`3 passed`と差分検査を実行した。
- [x] **長時間作業を専用タブへ分離。** ユーザ指定により、現タブの分割ペーンではなく、
  同一workspaceの専用タブを作って再利用する規則へ更新した。実際にタブ`w1:t3`・
  ペーン`w1:p5`をフォーカス非移動で作成し、文書参照テスト`3 passed`と差分検査を実行した。
