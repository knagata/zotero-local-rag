...
- [ ] **重いテストをオプトインにする** — フルスイート188秒のうち **131秒（70%）が
  `tests/test_ingestion_baseline.py` の5件**。しかもCIでは走らない（蔵書・Zotero・
  埋め込みモデル・ローカル生成ベースラインを要するため）。毎回3分待たされ、その大半は
  CIが保証しない検査という状態になっている。
  - pytestマーカーを付けて既定の実行から外す。既定は約50秒に戻る。
  - 取込・構造復元に触るときだけ明示的に走らせる運用にし、`CLAUDE.md` のコマンド節と
    `docs/development.md` に反映する。CIで強制される層は変わらない（元々走っていない）。
- [ ] **関数サイズのラチェット検査を置く** — 150行超が20個、400行超が7個
...
- [ ] **`docs/post-refactor-followups.md` を棚卸しする** — 47件まで膨らみ、完了済みと
  陳腐化したものが混ざっている。このままでは「誰も読み返さない記録」になり、
  `CLAUDE.md` に書いた腐り方をそのままなぞる。生きている課題だけに絞って優先度を付ける。
