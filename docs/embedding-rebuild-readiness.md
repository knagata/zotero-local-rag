...
|---|---|---|
| Rebuild code generation | HOLD | The scale pilot now rejects any nonempty pre-existing data plane before measurement, preventing a zero-write rerun from being reported as full throughput. Its verified implementation commit must be pinned here before M5. |
...
The data-plane and rebuild gates remain ready, but M5 is on hold until the
verified pilot-fix implementation commit is pinned above. It will still require
a separate explicit approval, including paid AI-TOC calls for eligible PDFs.
