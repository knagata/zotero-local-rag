...
|---|---|---|
| Rebuild code generation | HOLD | The scoped AI-TOC negative-cache refresh control has changed the ingestion runtime after `4b58d6b`. Its verified commit must be pinned here before M5. |
...
The data-plane and embedder gates remain ready, but M5 is on hold until the
verified ingestion runtime commit is pinned above. It still requires a separate
explicit approval to cross the destructive confirmation.
