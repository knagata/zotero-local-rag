...
文書構造からbottom-upに生成した採択済み・候補のLLM要約だけを検索索引（`__sum_node`）へ
格納します。抽出型（extractive）要約は表示用fallbackであり、索引対象外です。階層検索は
要約経由の候補探索と全体への直接段落検索を併用し、要約や索引が利用できない場合も直接検索を
availability fallbackとして維持します。
