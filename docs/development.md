...
**このネットが見ているのはループの21%だけ**（1,075実行行中235行）。3件の添付が通らない
分岐 — 画像ページのEPUB、抽出が本文を取りこぼしたPDF、空で返ったバッチ — は覆われて
いない。**括り出す前にそのブロックの到達率を見ること。** 見ていない箇所の変更は、ネットが
検証したのではなく異議を唱えなかっただけになる。
